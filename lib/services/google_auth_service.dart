...
class GoogleAuthService {
  ...
  static final GoogleSignIn _googleSignIn = GoogleSignIn(
    ...
    serverClientId:
        '266056580802-a2t9a90bf3776on3nhlapvhl37e2dcsk.apps.googleusercontent.com',
    // Forcer la sélection de compte à chaque fois
  ...
  static Future<User?> signInWithGoogle() async {
    try {
      ...
      if (kIsWeb) {
        ...
        // Pour Android/iOS, d'abord se déconnecter pour forcer la sélection de compte
        await _googleSignIn.signOut();

        // Puis se connecter avec sélection de compte
  ...
  // Switch Google Account - Force account selection
  static Future<User?> switchGoogleAccount() async {
    try {
      if (kDebugMode) print('GoogleAuthService: Switching Google account...');

      // Déconnexion complète
      await signOut();

      // Attendre un peu pour s'assurer que la déconnexion est complète
      await Future.delayed(const Duration(milliseconds: 500));

      // Reconnecter avec sélection de compte forcée
      return await signInWithGoogle();
    } catch (e) {
      if (kDebugMode) print('GoogleAuthService switch account error: $e');
      rethrow;
    }
  }

  // Force disconnect all accounts and clear cache
  static Future<void> forceSignOut() async {
    try {
      await Future.wait([
        _auth.signOut(),
        _googleSignIn.disconnect(), // Plus radical que signOut()
      ]);
      if (kDebugMode) print('GoogleAuthService: Force sign-out successful');
    } catch (e) {
      if (kDebugMode) print('GoogleAuthService force sign-out error: $e');
      rethrow;
    }
  }
}
